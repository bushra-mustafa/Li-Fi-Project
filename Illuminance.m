...
Adet=7.8E-7 ;  %detector physical area of a PD
...
FOV=30*pi/180; %FOV of a receiver
...
Nx=lx*10; Ny=ly*10;      % number of grid in the receiver plane
x=-lx/2:lx/Nx:lx/2;
y=-ly/2:ly/Ny:ly/2;
[XR,YR]=meshgrid(x,y);                 % receiver plane grid
D1=sqrt((XR-XT(1,1)).^2+(YR-YT(1,1)).^2+h^2);         % distance vector from source 1
...
P_rec=P_total.*H_A1.*Ts.*G_Con;    % received power 
P_rec_dBm=10*log10(P_rec);


%% 'Line of site Channel Gain
meshc(x,y,P_rec_dBm);
xlabel('X (m)');
ylabel('Y (m)');
zlabel('Received power (dBm)');
axis([-lx/2 lx/2 -ly/2 ly/2 min(min(P_rec_dBm)) max(max(P_rec_dBm))]);
title('Line of site Channel Gain');
%% Received Power
figure(2)
meshc(x,y,H_A1);
xlabel({'Room length','(meters)'});
ylabel ({'Room width','(meters)'});
zlabel ({'Luminous power','(Watt)'});
axis([-lx/2 lx/2 -ly/2 ly/2 min(min(H_A1)) max(max(H_A1))]);
